...
def test_process_timeout():
    ...
    with pytest.raises(ProcessTimeoutError, match="Timeout"):
        fs.run_command([sys.executable, "-c", "import time; time.sleep(10)"], shell=False, timeout=0.1)
    assert fs.active_processes == 0
...
def test_fetch_http_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(module, "validate_public_url", lambda *args, **kwargs: None)
    engine = SearchEngine(PanicController(), tmp_path)
...
def test_fetch_size_limit(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(module, "validate_public_url", lambda *args, **kwargs: None)
    engine = SearchEngine(PanicController(), tmp_path)
    engine._http = httpx.Client(
        transport=httpx.MockTransport(
            lambda _: httpx.Response(200, headers={"content-type": "text/plain"}, content=b"x" * 2_000_001)
        )
    )
...
def test_history_is_bounded(tmp_path):
    ...
    agent = Agent(Settings(max_history_messages=16), panic, client, engine, None)
    for _ in range(20):
        assert agent.run("hello") == "ok"
    assert len(agent.history) == 16  # la ventana manda sobre lo que se acumule
    agent.reset_history()
