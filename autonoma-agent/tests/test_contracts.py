...
def transport(client, response):
    client._client = httpx.Client(base_url='https://api.notrack.ai/v1/', transport=httpx.MockTransport(lambda _: response))
...
@pytest.mark.parametrize('data', [None, [], {}, {'choices': {}}, {'choices': ['x']},
    {'choices': [{'message': 'x'}]}, completion(42), completion(calls={}),
    completion(calls=[{}]), completion(calls=[{'type': 'function', 'id': '', 'function': {}}]),
...
@pytest.mark.parametrize('url', ['http://example.com', 'https://user:pass@example.com', 'https://example.com?q=1', 'https://example.com#x', 'file:///tmp'])
